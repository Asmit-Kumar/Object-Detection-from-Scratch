"""
Script to compare different pipeline test runs.
Parses JSON log files generated by the pipeline and calculates mean confidence metrics.
"""
import json
...
def load_run(filepath):
    """
    Load a pipeline test run from a JSON file and compute metrics.

    Args:
        filepath (str): Path to the JSON log file.

    Returns:
        dict: A dictionary containing mean confidence per digit, 
              bbox sources used, and total number of samples, 
              or None if file doesn't exist/is invalid.
    """
